...
public class StickToEnemy : MonoBehaviour
{
    ...
    [SerializeField] private RunnerController m_runner;
    ...
    {
        switch (collision.collider.tag == "Runner" && counter == 0 && m_myAnimator.enabled == true)
        {
            case true:
                counter = 1;
                transform.localPosition = new Vector3(0, 0, 0);
                transform.position = new Vector3(transform.position.x, transform.position.y, transform.position.z);
                m_runner.m_speed -= m_speedReduction;
    ...
    {
        switch (collision.collider.tag == "Runner" && counter == 1)
        {
            case true:
